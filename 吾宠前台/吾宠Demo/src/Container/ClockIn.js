                    ...
                    this.state.data.map((item)=>(
                        ...
                        <div key={item.id} style={{height: '100px',float:"left",marginLeft:"10%",marginTop:"5%"}}>
                            ...
                            <div style={{ color: '#888', fontSize: '14px'}}>
                                ...
                                <div style={{textAlign:'center'}}><button className='btn' onClick={()=>(this.daka(item.clockId))}>{item.clockName}</button><span style={{marginLeft:'10px'}} id='clock'>{item.clockedNum}</span><button className='btn' onClick={()=>(this.bianji(item.clockId))}><Link to={'/bianji/'+item.clockId+"&&"+this.state.dengluId} style={{color:'white'}}>编辑</Link></button></div>
